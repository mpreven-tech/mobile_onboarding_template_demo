...
export function HomeTypeScreen({
  onNext,
  onBack,
  selectedType,
  onSelect,
  ...
  // Get the first question from the brand kit's vertical preset
  const verticalPreset = VERTICAL_PRESETS[brandKit.vertical];
  const question = verticalPreset?.onboardingQuestions[0] || {
    id: 'default',
    ...
    <div className="h-full flex flex-col bg-white">
      ...
      <div className="px-6 mb-6">
        <div className="h-1.5 bg-gray-200 rounded-full overflow-hidden">
          <div 
            className="h-full w-[42%] rounded-full transition-all duration-300" 
      ...
      <div className="flex-1 px-6 pb-24 overflow-y-auto">
        <div className="grid grid-cols-2 gap-4">
          {question.options.map((type) => {
            const IconComponent = (Icons as any)[type.icon] || Icons.Circle;
